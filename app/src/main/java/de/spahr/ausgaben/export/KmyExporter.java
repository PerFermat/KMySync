...
public class KmyExporter {
    ...
    /**
     * Korrigiert bei geplanten Wertpapier-Umbuchungen (siehe {@code ScheduleMatch}) die Stückzahl im
     * Investment-Split der Planung auf den beim Bestätigen berechneten Wert ({@link
     * ScheduledAdvance#newShares}) und leitet daraus den passend gerundeten Kurs ab – der feste
     * Euro-Betrag der Planung ({@code value}) bleibt unverändert, wie es ihre „Schätzung"-Einstellung in
     * KMyMoney vorsieht.
     *
     * <p>Eigener Durchlauf statt Teil von {@link #applyScheduleAdvances}: dafür muss das betroffene
     * Unterkonto über {@link #doc} aufgelöst werden (Depot + Wertpapier-Id), während das Weiterstellen
     * von {@code postdate}/{@code lastPayment} reine Textarbeit ohne die Datei-Struktur ist.</p>
     */
    public String applyShareCorrections(String xml, List<ScheduledAdvance> advances) {
        String result = xml;
        if (advances == null) {
            return result;
        }
        for (ScheduledAdvance a : advances) {
            if (a.newShares == null || isEmpty(a.securityDepot) || isEmpty(a.securityKmyId)
                    || isEmpty(a.kmyId)) {
                continue;
            }
            Matcher block = Pattern.compile("<SCHEDULED_TX\\b[^>]*\\bid=\"" + Pattern.quote(a.kmyId.trim())
                    + "\"[^>]*>.*?</SCHEDULED_TX>", Pattern.DOTALL).matcher(result);
            if (!block.find()) {
                continue; // Regel nicht (mehr) in dieser Datei
            }
            String tx = block.group();
            String updated = applyShareCorrection(tx, a);
            if (!updated.equals(tx)) {
                result = result.substring(0, block.start()) + updated + result.substring(block.end());
            }
        }
        return result;
    }
    ...
    /** Der Wertpapier-Split innerhalb eines {@code <SCHEDULED_TX>}-Blocks, neu berechnet. */
    private String applyShareCorrection(String tx, ScheduledAdvance a) {
        String depotId = doc.depotId(a.securityDepot);
        String stockId = depotId == null ? null : stockAccountId(depotId, a.securityKmyId);
        if (stockId == null) {
            return tx; // Depot/Wertpapier nicht (mehr) in der Datei – lieber unverändert als falsch
        }
        Matcher splits = Pattern.compile("<SPLIT\\b[^>]*/>").matcher(tx);
        while (splits.find()) {
            String splitTag = splits.group();
            if (!splitTag.contains("account=\"" + stockId + "\"")) {
                continue;
            }
            Matcher sharesAttr = Pattern.compile("\\bshares=\"([^\"]*)\"").matcher(splitTag);
            if (!sharesAttr.find()) {
                return tx;
            }
            boolean negative = sharesAttr.group(1).trim().startsWith("-");
            double magnitude = Math.abs(a.newShares);
            String newShares = (negative ? "-" : "") + decimalFraction(magnitude, SHARE_SCALE);
            String updatedTag = splitTag.substring(0, sharesAttr.start(1)) + newShares
                    + splitTag.substring(sharesAttr.end(1));

            Matcher valueAttr = Pattern.compile("\\bvalue=\"([^\"]*)\"").matcher(updatedTag);
            long grossCents = valueAttr.find() ? Math.abs(valueToCents(valueAttr.group(1))) : 0;
            String newPrice =
                    priceFraction(grossCents, magnitude, doc.securityPricePrecision(a.securityKmyId));
            Matcher priceAttr = Pattern.compile("\\bprice=\"([^\"]*)\"").matcher(updatedTag);
            if (priceAttr.find()) {
                updatedTag = updatedTag.substring(0, priceAttr.start(1)) + newPrice
                        + updatedTag.substring(priceAttr.end(1));
            }
            return tx.substring(0, splits.start()) + updatedTag + tx.substring(splits.end());
        }
        return tx;
    }
